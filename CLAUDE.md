...
- **Waiting for `os.getpid()` is not enough, and this bit us in production.** A PyInstaller *onefile* exe is **two** processes: the bootloader parent that unpacks the archive into `%TEMP%\_MEIxxxxxx`, and the Python child that runs the app — and `os.getpid()` returns the *child*. Replacing the exe while the parent still holds it produced a `Failed to load Python DLL '…\_MEIxxxxxx\python312.dll'` box on the next launch (an interrupted unpack), and a 40-second gap in the log between "Kilépés" and the new "Indulás". The real precondition is not "is the process gone" but **"is the file replaceable"**, so the script now retries `move` in a loop (up to ~60 s), only `start`s after a *successful* move, and waits 3 s more before launching so the old instance's temp cleanup is finished. If the move never succeeds it exits without starting anything rather than launching a half-swapped binary.
