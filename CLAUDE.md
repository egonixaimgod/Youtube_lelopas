...
- **Shared messages are buffered in `kozos_naplo`** and replayed into every newly opened tab. `Alkalmazas._log()` only reaches the tabs that exist when it runs, so without the buffer a tab opened after startup had an empty log and no sign that the tools were ready.
...
- **Windows cannot overwrite a running exe**, so `frissites_telepites()` writes a small `.bat` that waits, then `move`s the downloaded file over `sys.executable`, `start`s it and deletes itself.
- **Launch the replacement with a cleaned environment — this is the one that actually bit us.** A frozen app's environment carries `_PYI_APPLICATION_HOME_DIR` (its `%TEMP%\_MEIxxxxxx` unpack dir), `_PYI_ARCHIVE_FILE` and `_PYI_PARENT_PROCESS_LEVEL`. The helper script inherits them, and so does the new exe it `start`s — whose bootloader then believes it is an already-unpacked *child* process and tries to load `python312.dll` from the **previous instance's** temp dir, which has just been deleted. The result is a `Failed to load Python DLL '…\_MEIxxxxxx\python312.dll' — LoadLibrary` box, the auto-restart never happening, and a suspicious multi-second gap in the log between "Kilépés" and the next "Indulás" (that gap is the user starting it by hand). `tiszta_kornyezet()` strips every `_PYI*` and `_MEIPASS2` variable before spawning the script. Verified end-to-end with a purpose-built onefile exe that replaces and relaunches itself: the restarted instance gets its own fresh `_MEI…` directory.
- The move itself is retried in a loop (up to ~60 s) rather than twice, and `start` only runs after a *successful* move — a onefile exe is two processes (bootloader parent + Python child) and `os.getpid()` is the child's, so the parent can still hold the file briefly. This is belt-and-braces, not the cause of the DLL error above.
