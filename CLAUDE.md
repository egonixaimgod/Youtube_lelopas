...
**Three modes** (`self.mod`): `zene` → `-x --audio-format mp3 --audio-quality 0`; `video` → resolution picker; `vr` → best video+audio with a `_3D_SBS` filename suffix. VR mode does *no* stereoscopic conversion — it only downloads at max quality and labels the file; the source is assumed to already be side-by-side.
...
- Hiding the quality list (`zene`/`vr` modes) must also drop its grid row's `weight`, or the freed space stays as a hole.
